 ...
 */
Config::Config(std::string const &path) { _servers = Parser(path)(); };
 ...
 */
directives_t &Location::getDirectives() { return _directives; };

/**
 * @brief   		check if request is allowed or not.
 * @param	req		the request to be checked.
 * @return 			bool indicate wither the request is allowed or
 * not.
 */
...
CGIMap_t &Location::getCGI() { return _CGIMap; }
