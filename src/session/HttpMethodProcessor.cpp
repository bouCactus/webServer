...
void HttpMethodProcessor::processGetRequest(HttpClient& client, servers_it& conf_S) {
...
void HttpMethodProcessor::processDeleteRequest(HttpClient& client, servers_it& conf_S) {
    HttpRequest &req = client.req;
...
void HttpMethodProcessor::processPostRequest(HttpClient& client, servers_it& conf_S) {
    HttpRequest &req = client.req;
    ...
    if (isLocationAllowUpload(req, conf_S)) {
        ...
        if (!resourceExists(requestedResource)) {
            ...
            executeCGIScriptAndGetResponse(requestedResource,
                                                  conf_S, client);
            return;
        }
